...
def compute_router_loss(
    ...
    for module in model.modules():
        if isinstance(module, LoraMoELinear) or isinstance(module, LoraMSPLinear):
            lb_loss = module.compute_balance_loss() if hasattr(module, "compute_balance_loss") else None
            z_loss = module.compute_z_loss() if hasattr(module, "compute_z_loss") else None
            spec_loss = module.compute_spec_loss(ground_rank,target_rank) if hasattr(module, "compute_spec_loss") else None
            mod_loss = module.compute_mod_loss() if hasattr(module, "compute_mod_loss") else None
            id_loss = module.compute_id_loss() if hasattr(module, "compute_id_loss") else None
