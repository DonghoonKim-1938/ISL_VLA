...
@dataclass
class AdaLoRAMoEConfig:
    r: int = 8  # low‑rank dim per expert
    k: float = 0.5  # rank truncation scale
...
class AdaMoELoRALinear(nn.Module):
    """A `nn.Linear` wrapped with *multiple* LoRA experts and a router.

    Args:
        base (nn.Linear): The frozen base projection.
        cfg (AdaLoRAMoEConfig): Hyper‑parameters.
    """

    def __init__(self, base: nn.Linear, cfg: AdaLoRAMoEConfig):
        super().__init__()
        assert isinstance(base, nn.Linear), TypeError("AdaMoELoRALinear expects an nn.Linear to wrap")
        ...
        self.A = nn.Parameter(torch.zeros(cfg.num_experts * cfg.r, in_f, dtype=base.weight.dtype))
        self.B = nn.Parameter(torch.zeros(out_f, cfg.num_experts * cfg.r, dtype=base.weight.dtype))
        # Init per LoRA paper
        ...
        self.router = nn.Linear(in_f, cfg.num_experts * cfg.r, dtype=base.weight.dtype)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:  # type: ignore
        ...
        # Router logits + gates
        ada_e_val = self.router(x_dp)
        ada_sigma = self._diag_embed(ada_e_val)

        lora_out = self._compute_lora_mix(x_dp, ada_sigma)

        return base_out + lora_out

    @staticmethod
    def _diag_embed(e_val: torch.Tensor) -> torch.Tensor:
        assert isinstance(e_val.shape, int)
        return torch.diag_embed(e_val) + torch.eye(e_val.shape, device=e_val.device)

    def _compute_lora_mix(self, x: torch.Tensor, ada_sigma: torch.Tensor) -> torch.Tensor:
        ...
        proj_r = torch.matmul(ada_sigma, proj_r)
...
def inject_ada_lora_moe(
    model: nn.Module,
    cfg: AdaLoRAMoEConfig | None = None,
    *,
    target_keywords: Iterable[str] | None = None,
    filter_fn: Callable[[str, nn.Module], bool] | None = None,
) -> Tuple[nn.Module, List[str]]:
    """Replace matching `nn.Linear` layers with `AdaMoELoRALinear` (in-place)."""

    cfg = cfg or AdaLoRAMoEConfig()
    wrapped: List[str] = []
    ...
    for name, module in model.named_modules():
        ...
        lora_moe_layer = AdaMoELoRALinear(module, cfg)
